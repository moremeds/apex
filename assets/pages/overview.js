...
function renderEtfDashboard(tickers, scoreHistory) {
  ...
  if (scoreHistory?.snapshots?.length > 1) {
    ...
    const allSyms = /* @__PURE__ */ new Set();
  ...
  for (const catKey of CATEGORY_ORDER) {
    ...
    if (!config) continue;
    const gridClass = config.card_style === "large" ? "hm-cards-large" : config.card_style === "mini" ? "hm-cards-mini" : "hm-cards-compact";
    let cardsHtml = "";
...
function renderStats(tickers) {
  ...
  container.innerHTML = `
    ...
    <span>Signals (24h): <strong>${totalSignals}</strong> \u{1F525}</span>
...
async function renderHeatmap(tickers) {
  ...
  const reservedHeight = 48 + 300 + 120;
  const treemapHeight = Math.max(500, window.innerHeight - reservedHeight);
  const hovertemplate = "<b>%{label}</b><br>Signals: %{customdata.signal_count}<br>Regime: %{customdata.regime}<br>Close: $%{customdata.close:.2f}<br>Daily: %{customdata.change:+.2f}%<br>Value: %{value:,.0f}<extra></extra>";
...
function updateColors() {
  ...
  const mode = document.getElementById("hm-color-by")?.value || "trending";
...
function updateSizes() {
  if (!treemapData || !treemapContainer) return;
  const mode = document.getElementById("hm-size-by")?.value || "market_cap";
