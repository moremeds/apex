...
function populateSymbolSelect(apex) {
  const select = document.getElementById("signals-symbol-select");
...
function renderChart(data) {
  ...
  try {
    ...
    if (mainEl) mainEl.innerHTML = '<div class="empty-state">Chart error: ' + err.message + "</div>";
  }
}
...
function renderConfluence(data, summary) {
  ...
  if (signals.length > 0) {
    const byInd = {};
    [...signals].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()).forEach((s) => {
      byInd[s.indicator || "unknown"] = s;
    });
    const active = Object.values(byInd);
    ...
    const rows = active.sort((a, b) => (a.indicator || "").localeCompare(b.indicator || "")).slice(0, 8).map((s) => `<div style="display:flex;justify-content:space-between;padding:3px 0;font-size:11px"><span style="color:${C.text_muted}">${s.indicator}</span><span style="color:${s.direction === "buy" ? C.success : s.direction === "sell" ? C.danger : C.text_muted}">${s.rule}</span></div>`).join("");
  ...
  if (Object.keys(priceLevels).length > 0 && lastClose) {
    ...
    let cards = "";
    for (const [ind, levels] of Object.entries(groups)) {
...
function renderDualMACD(data) {
  ...
  for (const row of rows) {
    ...
    const tacCell = tactical === "DIP_BUY" ? `<span style="color:#10b981;font-weight:600">DIP_BUY</span>` : tactical === "RALLY_SELL" ? `<span style="color:#ef4444;font-weight:600">RALLY_SELL</span>` : `<span style="color:${C.text_muted}">\u2014</span>`;
...
function renderTrendPulse(data) {
  ...
  let topBadge = curTop === "TOP_DETECTED" ? `<span style="color:#a855f7;font-weight:700">TOP_DETECTED</span>` : curTop === "TOP_ZONE" ? `<span style="color:#f59e0b;font-weight:700">TOP_ZONE</span>` : `<span style="color:${C.text_muted}">\u2014</span>`;
  const cards = `
    <div class="metric-cards" style="grid-template-columns:repeat(4,1fr)">
  ...
  for (const row of rows) {
    ...
    let bg = "transparent";
    if (entry) bg = rowBgColors.ENTRY;
    else if (swing !== "NONE") bg = rowBgColors[swing] || bg;
    else if (top === "TOP_DETECTED") bg = rowBgColors.TOP_DETECTED;
