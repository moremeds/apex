...
def run_comparison(
    ...
    from src.runners._pg_publish import publish_backtest_results

    run_id = f"compare-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    for strat in comparison_strategies.values():
        publish_backtest_results(
            run_id,
            strat["name"],
            comparison_symbols,
