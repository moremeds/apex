...
class HistoricalStoreDataFeed(DataFeed):
    ...
    def _get_store(self):
        ...
        if self._store is None:
            from ..stores.parquet_historical_store import ParquetHistoricalStore

            self._store = ParquetHistoricalStore(base_dir=self._base_dir)
        return self._store

    async def load(self) -> None:
        """Load data from ParquetHistoricalStore."""
        self._bars.clear()
        ...
        for symbol in self._symbols:
            file_path = store.get_file_path(symbol, self._bar_size)
