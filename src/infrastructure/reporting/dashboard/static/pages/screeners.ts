/**
 * Screeners Page — Tabulator tables for Momentum + PEAD candidates.
 * Uses window.Tabulator (loaded eagerly via regular script tag).
 *
 * Momentum tab: 5 summary cards + sortable table with quality/regime badges.
 * PEAD tab: 3 summary cards + tier sections + expandable detail rows + methodology.
 */

let momentumTable: any = null;
let peadTable: any = null;
let screenersData: any = null;

// ─── Formatters ─────────────────────────────────────────────────────────────

function qualityBadge(val: string): string {
...
function regimeBadge(val: string): string {
  if (!val) return '\u2014';
  const rc = val.toLowerCase();
  return `<span class="regime-badge ${rc}">${val}</span>`;
}

function qualityFormatter(cell: any): string {
  return qualityBadge(cell.getValue());
}

function regimeFormatter(cell: any): string {
  return regimeBadge(cell.getValue());
}

function symbolFormatter(cell: any): string {
...
function pctFormatter(cell: any): string {
  const val = cell.getValue();
  if (val == null) return '\u2014';
  return `${(val * 100).toFixed(1)}%`;
}

function dollarFormatter(cell: any): string {
  const val = cell.getValue();
  if (val == null) return '\u2014';
  return `$${val.toFixed(2)}`;
}

function bigNumberFormatter(cell: any): string {
  const val = cell.getValue();
  if (val == null) return '\u2014';
...
function floatFormatter(decimals: number) {
...
function earnRetFormatter(cell: any): string {
  const v = cell.getValue();
  if (v == null) return '\u2014';
...
const PEAD_COLUMNS = [
  ...
  { title: 'Vol', field: 'earnings_day_volume_ratio', hozAlign: 'right', width: 70, formatter: (c: any) => { const v = c.getValue(); return v != null ? v.toFixed(1) + 'x' : '\u2014'; } },
  { title: 'Rev', field: 'revenue_surprise_pct', hozAlign: 'right', width: 70, formatter: pctFormatter },
  { title: 'Quality', field: 'quality_label', formatter: qualityFormatter, formatterParams: { html: true }, width: 90 },
  { title: 'Gap Held', field: 'gap_held', hozAlign: 'center', width: 80, formatter: (c: any) => { const v = c.getValue(); return v ? '<span style="color:var(--positive)">Yes</span>' : '<span style="color:var(--negative)">No</span>'; }, formatterParams: { html: true } },
  { title: 'Entry', field: 'entry_price', formatter: dollarFormatter, hozAlign: 'right', width: 80 },
  { title: 'Stop %', field: 'stop_loss_pct', hozAlign: 'right', width: 75, formatter: (c: any) => { const v = c.getValue(); return v != null ? v.toFixed(1) + '%' : '\u2014'; } },
  { title: 'Target %', field: 'profit_target_pct', hozAlign: 'right', width: 80, formatter: (c: any) => { const v = c.getValue(); return v != null ? v.toFixed(1) + '%' : '\u2014'; } },
...
function setupTabs(): void {
  document.querySelectorAll('#page-screeners .tab-btn').forEach((btn: Element) => {
    btn.addEventListener('click', () => {
      const tab = (btn as HTMLElement).dataset.tab;
      document.querySelectorAll('#page-screeners .tab-btn').forEach((b: Element) =>
        b.classList.toggle('active', (b as HTMLElement).dataset.tab === tab));
      document.querySelectorAll('#page-screeners .screener-panel').forEach((p: Element) =>
        p.classList.toggle('active', p.id === `screener-${tab}`));
    });
  });
}
...
function addExportButton(containerId: string, table: any, label: string): void {
...
function renderMomentumSummary(data: any): void {
  ...
  const avgMom = total > 0
    ? (candidates.reduce((s: number, c: any) => s + (c.momentum_12_1 || 0), 0) / total * 100).toFixed(1) + '%'
    : '\u2014';
  const avgFip = total > 0
    ? (candidates.reduce((s: number, c: any) => s + (c.fip || 0), 0) / total).toFixed(3)
    : '\u2014';
...
function renderPeadSummary(data: any): void {
  ...
  const qualityCounts: Record<string, number> = { STRONG: 0, MODERATE: 0, MARGINAL: 0 };
  candidates.forEach((c: any) => {
    const q = (c.quality_label || '').toUpperCase();
    if (q in qualityCounts) qualityCounts[q]++;
  });
  const avgQuality = total > 0
    ? `${qualityCounts.STRONG}S / ${qualityCounts.MODERATE}M / ${qualityCounts.MARGINAL}W`
    : '\u2014';
...
function renderMomentumTable(data: any): void {
  const candidates = data?.candidates || [];
  ...
  const ranked = candidates.map((c: any, i: number) => ({ rank: i + 1, ...c }));
...
function renderPeadTable(data: any): void {
  const candidates = data?.candidates || [];
...
function renderPeadMethodology(): void {
...
export async function init(_apex: any): Promise<void> {
  setupTabs();
...
export function update(): void {
  // Tables persist — no re-render needed on re-visit
}
