...
- **`src/api/routes/lake.py`** (`/v1/lake/asset-classes`, `/v1/lake/status`, `/v1/lake/coverage`, `/v1/lake/silver-revisions[/{n}]`, `/v1/lake/pit-revisions[/{n}]`, `/v1/security/{symbol}`, `/v1/futures/{root}/contracts`, `/v1/{asset_class}/{symbol}/gaps`) is likewise registered before `instruments` — its three- and two-segment paths would otherwise be swallowed by `/v1/{asset_class}/{symbol}`. There is no PIT `current` route: Livewire's `pit-revisions/current.json` is one pointer shared by every index, so apex never resolves an index's PIT revision through it.
- **`listing=delisted` and `listing=any` serve bars** from `bronze-delisted/`, raw only. `price_mode=adjusted` over that tree is a `400 adjusted_not_supported`, never a silent raw fallback. A dual-resident ticker returns the union with the live tree winning every shared America/New_York trading date, labelled `listing_status: "dual"`. Presence in both trees does not by itself mean two issuers — check `GET /v1/equity/{symbol}/delisting` (security-master intervals) before computing a return across the seam. `src/application/lake/guards.py::check_listing` owns the resolution.
- **Bars accept a `silver_revision` or `pit_revision` pin** (mutually exclusive, both imply `price_mode=adjusted`, and both require `asset_class=equity`, `timeframe=1d`, `listing=listed` — an immutable-history claim cannot rest on mutable Bronze intraday or the unadjusted delisted archive). `pit_revision` serves only the member's PIT scope; a window outside it is `400 invalid_parameter`, a window spanning two `security_id`s is `409 ambiguous_symbol`. `src/application/lake/bars.py::check_pins` / `_pit_bars`. Bulk bars (`GET /v1/equity/bars`) live in the sibling `src/application/lake/bulk.py` (`query_bulk_bars`, `normalize_symbols`, `BulkResult`/`BulkSeries`), not in `bars.py`.
- **Every parquet read runs under a per-query deadline.** `src/infrastructure/adapters/livewire/parquet_reads.py::LakeDb` executes each DuckDB read on its own handle in a worker thread; on expiry (`APEX_LAKE_QUERY_TIMEOUT_SECONDS`, default 30) it interrupts only that handle and raises `QueryTimeout`, which the lake layer turns into `LakeError("query_timeout", ...)` → `504`. On the bulk route a timed-out symbol lands in `missing` rather than failing the whole request.
